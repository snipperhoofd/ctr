...
# Build a list of modules based on the 'pathway module' "Mineral and organic ion transport system" modules
names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Mineral and organic ion transport system")]))
M_I_module_list<- as.list(names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Mineral and organic ion transport system")])))
names(M_I_module_list)<-names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Mineral and organic ion transport system")]))

for (i in 1:length(M_I_module_list)) {
  M_I_module_list[[i]]<-KO_modules_table2$V4[which(KO_modules_table2$V2==M_I_module_list[i])]
}

# vector of all KOs in module_list
M_I_KOs<-unlist(module_list, use.names=FALSE)
...
matrix_features_B <- new("General_features",
                       ...
                       Pairwise_Bin_Array_Presence = Presence_Absence_Matrix(RNAseq_Annotated_Matrix_B),
                       no_annotation = which(names((table(RNAseq_Annotated_Matrix_B$KO)))==""),
                       All_KOs = names(table(RNAseq_Annotated_Matrix_B$KO))[-no_annotation]
                       )
...
ptm <- proc.time()
Presence_Absence_Matrix(RNAseq_Annotated_Matrix_B,5)
proc.time() - ptm

# its faster to take the sd than look if there is just one value in a table
ptm <- proc.time()
for (i in 1:1000) {
...
ptm <- proc.time()
Random_Background_Module_Distances_6_BR<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_BR_default_bin, matrix_features_BR, Z_scores_BR, 6, 10000)
proc.time() - ptm

Random_Background_Module_Distances_7_BR<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_BR_default_bin, matrix_features_BR, Z_scores_BR, 7, 10000)
Random_Background_Module_Distances_8_BR<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_BR_default_bin, matrix_features_BR, Z_scores_BR, 8, 10000)

Random_Background_Module_Distances_6_B<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_B_default_bin, matrix_features_B, Z_scores_B, 6, 10000)
Random_Background_Module_Distances_7_B<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_B_default_bin, matrix_features_B, Z_scores_B, 7, 10000)
Random_Background_Module_Distances_8_B<-Background_Distribution_Modules(RNAseq_Annotated_Matrix_B_default_bin, matrix_features_B, Z_scores_B, 8, 10000)
...
M_I_pairwise_KO_distances <- P_NRED_Distance_Function(RNAseq_Annotated_Matrix_BR_default_bin, Z_scores_BR, matrix_features_BR, M_I_module_list)
...
polymers_pairwise_KO_distances <- P_NRED_Distance_Function(RNAseq_Annotated_Matrix_BR_default_bin, Z_scores_BR, matrix_features_BR, polymers)
polymers_clustering_results_P_NRED <-cluster_func(RNAseq_Annotated_Matrix_BR_default_bin, all_pairwise_KO_distances$Composite_Z_Score, matrix_features_BR, polymers)
