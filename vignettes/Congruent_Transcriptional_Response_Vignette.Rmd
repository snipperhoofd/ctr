...
# Build a list of modules based on the 'pathway module' "Central carbohydrate metabolism" modules
names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Central carbohydrate metabolism")]))
CCM_module_list<- as.list(names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Central carbohydrate metabolism")])))
names(CCM_module_list)<-names(table(KO_modules_table2$V2[which(KO_modules_table2$V1=="Central carbohydrate metabolism")]))

for (i in 1:length(CCM_module_list)) {
  CCM_module_list[[i]]<-KO_modules_table2$V4[which(KO_modules_table2$V2==CCM_module_list[i])]
}


# vector of all KOs in module_list
M_I_KOs<-unlist(M_I_module_list, use.names=FALSE)
polymer_KOs<-unlist(polymers, use.names=FALSE)
CCM_KOs<-unlist(CCM_module_list, use.names=FALSE)
```
...
M_I_pairwise_KO_distances <- P_NRED_Distance_Function(RNAseq_Annotated_Matrix_B_default_bin, Z_scores_B, matrix_features_B, M_I_KOs)
M_I_clustering_results_P_NRED <-cluster_func(RNAseq_Annotated_Matrix_B_default_bin, M_I_pairwise_KO_distances$combined, matrix_features_B, M_I_module_list)
