...
ave_Z_score_Func <- function(combined_scores_array){
  ave_Z_score<-matrix(NA,ncol=length(high_quality_bins),nrow=length(high_quality_bins))
  for (x in 1:(dim(combined_scores_array)[1]-1)) {
    for (y in (x+1):dim(combined_scores_array)[2]) {ave_Z_score[x,y]<-mean(combined_scores_array[x,y,],na.rm=TRUE)}}
  return(ave_Z_score)
}
