...
GetFeatures <- function(RNAseq_Annotated_Matrix,
  ...
  position_of_genome_A <-
    which(RNAseq_Annotated_Matrix$Bin == random_genomes[1])
  position_of_genome_B <-
    which(RNAseq_Annotated_Matrix$Bin == random_genomes[2])

  # Identify the rows in the original matrix corresponding to a KO
  KO_positions <-
    which(RNAseq_Annotated_Matrix$KO == random_module[j])

  # Find intersection between the genome lists and the KO list
  position_of_kegg_enzyme_A <-
    intersect(KO_positions, position_of_genome_A)
  position_of_kegg_enzyme_B <-
    intersect(KO_positions, position_of_genome_B)
  ...
  return(
    list(
      "position_of_genome_A" = position_of_genome_A,
...
RandomDistances <- function(pairwiseDistances, Z_scores) {
  # Convert to Z scores
  Zscore_pairwise_gene_correlation <-
    (pairwiseDistances$pairwise_correlation - Z_scores$mu[2]) / Z_scores$sd[2] # need to inverse PCC
  Zscore_pairwise_gene_euclidean <-
    (pairwiseDistances$pairwise_euclidean - Z_scores$mu[6]) / Z_scores$sd[6]

  #If there is a tie, the first minimum is taken
  best_scoring_pair <-
    which.min((1 - Zscore_pairwise_gene_correlation) + (Zscore_pairwise_gene_euclidean))


  Random_Pearson_Distances <-
    pairwiseDistances$pairwise_correlation[best_scoring_pair]
  Random_Euclidean_Distances <-
    pairwiseDistances$pairwise_euclidean[best_scoring_pair]


  return(
    list(
      "Random_Pearson_Distances" = Random_Pearson_Distances,
...
comparePairwise <- function(position_of_kegg_enzyme_A,
           position_of_kegg_enzyme_B,
           RNAseq_Annotated_Matrix,
           matrix_features) {
    l_position_of_kegg_enzyme_A <- length(position_of_kegg_enzyme_A)
    l_position_of_kegg_enzyme_B <- length(position_of_kegg_enzyme_B)
    max_pairwise_gene_correlation <-
      matrix(NA, nrow = l_position_of_kegg_enzyme_A,
    ...
    for (m in 1:l_position_of_kegg_enzyme_A) {
      for (n in 1:l_position_of_kegg_enzyme_B) {
        ...
        correlation <-
          cor(
            as.numeric(RNAseq_Annotated_Matrix[position_of_kegg_enzyme_A[m],
        ...
        subtracted_lists <-
          RNAseq_Annotated_Matrix[position_of_kegg_enzyme_A[m], matrix_features@RS:matrix_features@RE] -
          RNAseq_Annotated_Matrix[position_of_kegg_enzyme_B[n], matrix_features@RS:matrix_features@RE]
        max_pairwise_gene_euclidean[m, n] <-
          sqrt(sum(subtracted_lists * subtracted_lists))
      }
    }
    return(
      list(
        "pairwise_correlation" = max_pairwise_gene_correlation,
