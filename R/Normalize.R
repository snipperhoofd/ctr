...
defaultRNA_Normalize <- function(RNAseq_Annotated_Matrix, matrix_features){
  ...
  sum_reads_per_genome_matrix <- reads_perBin_perSample(RNAseq_Annotated_Matrix, matrix_features)
  ...
  max_count_per_bin <- apply(sum_reads_per_genome_matrix, 1, max)
  normalized_sum_reads_per_genome_matrix <- sum_reads_per_genome_matrix / max_count_per_bin
  ...
  RNAseq_Annotated_Matrix[, matrix_features@SS : matrix_features@SE] <- log(RNAseq_Annotated_Matrix[, matrix_features@SS:matrix_features@SE], 2)
...
reads_perBin_perSample <- function(RNAseq_Annotated_Matrix, matrix_features){
  # Calculate the number of reads mapped to each bin in each sample
  sum_reads_per_genome_matrix<-matrix(NA,nrow=length(matrix_features@high_quality_bins),
                                      ncol=length(matrix_features@sample_names))
